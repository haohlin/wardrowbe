...
class AIService:
    ...
    def _parse_tags_from_response(self, response_text: str) -> ClothingTags:
        ...
        tags.colors = validate_list(normalize_list(data.get("colors", [])), VALID_COLORS)
        ...
        tags.style = validate_list(normalize_list(data.get("style", [])), VALID_STYLES)
        tags.season = validate_list(normalize_list(data.get("season", [])), VALID_SEASONS)
        ...
        if isinstance(features, list):
            tags.features = [str(value).strip() for value in features if str(value).strip()]
